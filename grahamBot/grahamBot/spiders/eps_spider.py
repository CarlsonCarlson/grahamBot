...
class EPSSpider(scrapy.Spider):
    ...
    def __init__(self, name, ticker, *args, **kwargs):
    ...
    def parse(self, response):
        ...
        complete_file_name = os.path.join(r'C:\Users\Carlson\PycharmProjects\grahamBot\written_files',
                                          filename)
        # source is a list
        ...
        year_eps_df = pd.DataFrame(np.column_stack([year_list, eps_list]),
                                   columns=['Year', 'EPS'])
        # Write file mainly for debugging
